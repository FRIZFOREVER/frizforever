<!-- GitHub Profile README for Friz | generated with your preferences -->

<!-- Replace FRIZFOREVER, fill placeholders like [ADD LINK], and delete any sections you don't want. -->

# Senior ML Engineer: NLP | RAG | Agents

![Capsule](https://capsule-render.vercel.app/api?type=waving\&height=160\&text=Friz\&fontAlign=50\&desc=RAG%20%7C%20Agents%20%7C%20NLP\&descAlign=50)

I am focused on building **local‑first RAG and agentic systems** for production 

My core prioritises are: 
- reproducibility
- observability
- clean engineering over hype

---

<!-- Contact / quick links -->

**Contact**

* Email: [kirillkhromovml@gmail.com](mailto:kirillkhromovml@gmail.com)
* LinkedIn: [LinkedIn/in/frizforever](https://www.linkedin.com/in/frizforever/)
* Time zone: GMT+3

<!-- Widgets (active) -->

<!-- ![GitHub Stats](https://github-readme-stats.vercel.app/api?username=FRIZFOREVER\&show_icons=true) -->
![Visitors](https://komarev.com/ghpvc/?username=FRIZFOREVER\&style=flat-square\&label=Visitors)
![Last Updated](https://img.shields.io/github/last-commit/FRIZFOREVER/FRIZFOREVER?label=last%20updated)

---

## Focus & Goals

* **Core focus:** RAG ▸ Agents ▸ NLP
* **Current goal:** Personal startup development
* **Passion:** Everything about RAG
* **Open to:** Work · Contribute · Contracting · Experience exchange

---


### 🧰 Toolbox
<div align="left">
  <img src="https://skillicons.dev/icons?i=neovim,linux,docker,git,postgres,py,fastapi,pytorch,sklearn,grafana,pytest&perline=10" />
</div>

<!-- OS / Env -->

![Fedora](https://img.shields.io/badge/Fedora-42-294172?logo=fedora\&logoColor=white)
![EndeavourOS](https://img.shields.io/badge/EndeavourOS-BC5CFF?logo=endeavouros\&logoColor=white)
![Hyprland](https://img.shields.io/badge/Hyprland-Wayland-0E7490)
![uv](https://img.shields.io/badge/uv-dependency%20manager-333333)
![Uvicorn](https://img.shields.io/badge/Uvicorn-ASGI-121212)

<!-- General ML -->

![NumPy](https://img.shields.io/badge/NumPy-013243?logo=numpy&logoColor=white)
![SciPy](https://img.shields.io/badge/SciPy-8CAAE6?logo=scipy&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?logo=pandas&logoColor=white)
![XGBoost](https://img.shields.io/badge/XGBoost-FF4A00)
![CatBoost](https://img.shields.io/badge/CatBoost-FFCC00)


<!-- RAG/Agents stack -->

![LangChain](https://img.shields.io/badge/LangChain-🦜️🔗-0E9F6E)
![LangGraph](https://img.shields.io/badge/LangGraph-Orchestration-0F172A)
![Transformers](https://img.shields.io/badge/Transformers-HF-FFB000?logo=huggingface\&logoColor=black)
![FAISS](https://img.shields.io/badge/FAISS-Vector%20Index-0052CC)
![Qdrant](https://img.shields.io/badge/Qdrant-Vector%20DB-FF4D4D)
![SGLang](https://img.shields.io/badge/SGLang-Server-111827)
![Ollama](https://img.shields.io/badge/Ollama-Local%20LLMs-2D333A)
![vLLM](https://img.shields.io/badge/vLLM-Server-1E90FF)
![PEFT](https://img.shields.io/badge/PEFT-LoRA-8A2BE2)
![Unsloth](https://img.shields.io/badge/Unsloth-LLM%20fine--tuning-4F46E5)

<!-- Databases / Storage -->

![PostgreSQL](https://img.shields.io/badge/Postgres-4169E1?logo=postgresql\&logoColor=white)
![MinIO](https://img.shields.io/badge/MinIO-S3-ef4444)

<!-- Observability / Evals -->

![LangSmith](https://img.shields.io/badge/LangSmith-2E2E2E)
![pytest](https://img.shields.io/badge/pytest-tests-0A9EDC)

---

## RAG & Agentic Approach

* **Retrieval strategies:** RAPTOR · CRAG · HyDE · RAG‑Fusion (plus hybrid baselines)
* **Embeddings:** Qwen3‑Embedding · Sentence‑BERT (sBERT)
* **Re‑ranking:** Qwen3‑Reranker
* **Chunking options:** classic (fixed/overlap) · structure‑based · semantic‑based · ML/LLM‑guided
* **Agent loop:** ReAct
* **Reasoning/guardrails:** Schema‑guided reasoning (LLM_under_the_hood, Neural_Covalski, RU)

---

## Evals & QA Quality

* **Tooling:** RAGChecker · TruLens · RuRAGE
* **Practices:** unit tests, dataset versioning, regression suites, qualitative spot‑checks

---

## Experience & Education

* Graduated from university
* 5+ years of work experience

---

## Now

* Currently working on my **startup project**

---

## Dev Setup (for the curious)

* OS: Arch • EndeavourOS
* Shell: zsh
* Editor: Neovim
* Containers: Docker, Compose

---

## Collaboration

* Language: English | Russian
* Typical response time: within 24 hours
* Preferred comms: Programming/Engineering → Telegram
* Code reviews: small PRs; **rebase** when merging into feature branches; **merge** when merging into DEV / Test / Master

---

## Reading / Survey

* LIMI: *Less is More for Agency* — arXiv: [https://arxiv.org/abs/2509.17567](https://arxiv.org/abs/2509.17567)

---

### Licence

All my public code is **MIT licensed**.

<!--
Future: Featured Projects (keep hidden for now)
- [PROJECT_1_NAME] — value proposition, highlights, link
- [PROJECT_2_NAME]
- [PROJECT_3_NAME]
-->
